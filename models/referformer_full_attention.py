...
from .segmentation import CrossModalFPNDecoder, VisionLanguageFusionModule, LanguageVisionFusionModule
...
# rearrange函数：维度重置， repeat函数：复制指定层
...
class ReferFormer(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        if num_feature_levels > 1:
            num_backbone_outs = len(backbone.strides[-3:])  # 倒数三层
            input_proj_list = []
        ...
        self.text_fusion_module = LanguageVisionFusionModule(d_model=hidden_dim, nhead=8)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-3:], pos[-3:])):
            ...
            text_word_features = self.text_fusion_module(tgt=text_word_features,
                                                         memory=src_proj_l,
